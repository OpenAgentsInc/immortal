...
jq -e '
  ...
  ([.extensions[].id] == ["lnd", "elementsd", "arkd"]) and
  ([.extensions[].issue] == [29, 27, 20]) and
  ([.extensions[].hook_environment] == ["IMMORTAL_LAB_LND_HOOK", "IMMORTAL_LAB_ELEMENTSD_HOOK", "IMMORTAL_LAB_ARKD_HOOK"]) and
...
scripts/lab-extensions.sh manifest lnd | jq -e '.issue == 29 and .state == "hook-only"' >/dev/null
...
mkdir -p "${test_dir}/lab/extensions/elementsd"
touch "${test_dir}/lab/extensions/elementsd/unowned"
set +e
IMMORTAL_LAB_DIR="${test_dir}/lab" scripts/lab-extensions.sh down elementsd >"${test_dir}/unowned-down" 2>&1
unowned_down_status="$?"
set -e
test "${unowned_down_status}" -eq 1
test -f "${test_dir}/lab/extensions/elementsd/unowned"

hook="${test_dir}/extension-hook"
printf '%s\n' \
  '#!/usr/bin/env bash' \
  'set -euo pipefail' \
  'test "${IMMORTAL_LAB_EXTENSION_ID}" = lnd' \
  'test "${IMMORTAL_LAB_EXTENSION_ISSUE}" = 29' \
  'test -n "${IMMORTAL_LAB_EXTENSION_RUN_ID}"' \
  'printf "%s" "${IMMORTAL_LAB_EXTENSION_PORTS_JSON}" | jq -e ".rpc == 19009" >/dev/null' \
  'case "$1" in' \
...
IMMORTAL_LAB_DIR="${test_dir}/lab" IMMORTAL_LAB_LND_HOOK="${hook}" \
  scripts/lab-extensions.sh up lnd >/dev/null
IMMORTAL_LAB_DIR="${test_dir}/lab" IMMORTAL_LAB_LND_HOOK="${hook}" \
  scripts/lab-extensions.sh status lnd | grep -q hook-active
IMMORTAL_LAB_DIR="${test_dir}/lab" IMMORTAL_LAB_LND_HOOK="${hook}" \
  scripts/lab-extensions.sh down lnd >/dev/null
test ! -e "${test_dir}/lab/extensions/lnd"
