...
use immortal_core::domain::{
    ...
    MKT_STATUS_KIND, MKT_SWP_INTENT_ACK_KIND, MKT_SWP_PROFILE_ID, MKT_SWP_PROFILE_VERSION,
    MktHardeningErrorCode, MktHardeningRecord, MktHardeningRecordKind, MktProfileSupport, Tag,
    validate_mkt_hardening_event, validate_mkt_private_with_profiles,
};
use serde::{Deserialize, Serialize};
use serde_json::{Value, json};
use sha2::{Digest, Sha256};

const JOURNAL_SCHEMA: &str = "openagents.mkt-swp.intent-journal.v1";
...
impl ProviderIntentJournal {
    ...
    pub fn restore(bytes: &[u8]) -> Result<Self, ProviderHardeningError> {
        let journal: Self = serde_json::from_slice(bytes).map_err(|detail| {
            error(
                ProviderHardeningErrorCode::Persistence,
    ...
    fn validate_snapshot(&self) -> Result<(), ProviderHardeningError> {
        ...
        for (scope, binding) in &self.bindings {
            ...
            if binding.intent.kind != MKT_ORDER_KIND && binding.effect_attempt_claimed {
                return Err(error(
                    ProviderHardeningErrorCode::Persistence,
                    "read-only Re-drive claimed an effect attempt",
                ));
            }
        }
