...
| Funded browser demo: loopback-only exact-origin regtest callback over the existing wallet harness; typed engine effect/session/digest binding; closed Bitcoin-funding and Lightning-payment methods; durable exact receipt replay after adapter replacement; provider Status separated from independently verified rail evidence; settled presentation withheld until terminal verification; and real submarine/reverse execution without exposing node or wallet credentials | `tests/fixtures/lab/browser-demo-v1.json`, `crates/immortal-lab/src/browser_demo.rs`, `scripts/dev-funded-browser-demo.sh`, `scripts/test-browser-demo-funded.sh`, and [`browser-demo-funded.md`](browser-demo-funded.md) |
| Public-regtest gateway: separate loopback-behind-TLS process and dependency graph; exact HTTPS Origin and trusted proxy client-IP policy; 256-bit per-session capabilities stored only as digests; signed revision/provider/quota manifests; closed engine-derived Bitcoin/Lightning effects; atomic admission; durable byte-identical receipt replay through replacement and concurrency; expiry/revocation/rate limits; duplicate-JSON and custody-output refusal | `tests/fixtures/lab/public-regtest-gateway-v1.json`, `crates/immortal-public-regtest-gateway`, `scripts/test-public-regtest-gateway.sh`, and [`public-regtest-gateway.md`](public-regtest-gateway.md) |
