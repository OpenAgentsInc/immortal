...
| Adversarial-lab #18 closed-world contract: exact isolated two-provider/two-relay topology, regtest-only tiny-expiry profile, 33 routing/failure/doomsday/cooperative cases, independent evidence views, retained-record limits, cleanup laws, and explicit claim exclusions. This row proves manifest completeness only; executable process evidence is added by the issue-#18 runner | `tests/fixtures/lab/adversarial-v1.json` and `scripts/test-lab-adversarial-manifest.sh` |
