...
| MKT-SWP Ark verification and client exit: exact Arkade/Bark operator identity and public-policy binding, bounded connected transaction-tree or transaction-chain traversal, observed-root authority, Taproot signature and VTXO path verification, fully signed pre-funded exit validation, external-artifact persistence before transfer, digest/reference-only snapshots, and ordered keyless Esplora recovery | `tests/fixtures/nipmkt/ark-rail-v1.json`, `crates/immortal-core/src/ark.rs`, `crates/immortal-core/tests/ark.rs`, `crates/immortal-client/src/ark.rs`, and `crates/immortal-client/tests/ark.rs` |
