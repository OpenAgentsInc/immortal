...
| Boltz released-client compatibility handoff: off-by-default exact-digest activation, safe origin construction including exact 64-ID status and parsed BOLT11 bounds, bounded external-provider `307`, sensitive body non-read proof, submarine-finalize route, script-path profile, exact 19-call inventory, dependency-free Go/web prepare→bilateral-Contract/script-exit→broadcast source gates, and separate 0/53 endpoint versus 0/19 dependent-call accounting | `tests/fixtures/nipmkt/boltz-facade-v2.json`, `tests/fixtures/nipmkt/boltz-client-adapters-v1.json`, `crates/immortal-core/tests/boltz_compat.rs`, `adapters/`, `scripts/test-boltz-client-adapters.sh`, relay unit tests, `docs/protocol/boltz-facade.md`, and the deterministic contract export. The external provider API and adapted-client process gate remain incomplete and are not claimed. |
...
| Funded provider runtime: reserve-gated exact reverse funding precommitment, production client verify-before-fund and `ExitPackage`, BIP-341/342 claim/refund construction bytes, exclusive chain deadlines, held-HTLC refusal/cancellation, cooperative refund-watch retirement, signer-local terminal Close, and deterministic closed-limit provider contract | `tests/fixtures/provider/provider-runtime-v1.json`, `tests/fixtures/provider/settlement-construction-v1.json`, `crates/immortal-provider/tests/provider_settlement.rs`, funded-mode unit tests, `crates/immortal-lab/tests/funded_live.rs`, `scripts/export-provider-contract.sh --check`, and `scripts/test-provider-funded.sh` (local three-journey plus harness-restart gate passed 2026-08-04 on macOS 26.4 arm64) |
| Adversarial-lab provisioning: opt-in-RBF creation/replacement, two provider-owned CLN nodes with mandatory probed hold RPCs, a separate wallet CLN node, balanced three-edge topology, loopback two-relay/two-provider allocations, ownership-checked teardown, and hook-only LND/elementsd/arkd registry entries without credential transfer | `tests/fixtures/lab/provisioning-v1.json`, `scripts/lab-bitcoind.sh`, `scripts/lab-cln.sh`, `scripts/lab-extensions.sh`, `scripts/lab-topology.sh`, and `scripts/test-lab-provisioning.sh` (static/fixture gate; clean-machine funded execution remains required by #32) |
...
The three-journey bitcoind/CLN process gate passed locally on 2026-08-04 on
macOS 26.4 arm64 with the exact success result `test-provider-funded:
submarine, reverse, and noncooperative refund passed`. This local regtest result
does not constitute Debian or live deployment evidence; issue #19 owns that
proof.
