...
| MKT-SWP transport-neutral requester execution: exact external signing requests, bilateral RFC 8785-compatible contract binding, submarine/reverse/chain verify-before-fund typestate, per-signer sequence gaps/forks, wallet-owned claim/refund signing, idempotent effect replay, crash restore, doomsday recovery, keyless pre-signed Esplora broadcast, and recursive custody tripwires | `tests/fixtures/nipmkt/swp-client-engine-v1.json`, `tests/mkt_swp_client.rs`, `docs/protocol/mkt-swp-client.md`, and `scripts/test-swp-verification.sh` on native and `wasm32-unknown-unknown` |
