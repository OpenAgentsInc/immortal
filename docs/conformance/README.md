...
| MKT-SWP Ark verification, client exit, and reserve durability: exact Arkade/Bark operator identity and public-policy binding, bounded connected transaction-tree or transaction-chain traversal, observed-root authority, Taproot signature and VTXO path verification, fully signed pre-funded exit validation, external-artifact persistence before transfer, digest/reference-only snapshots, ordered keyless Esplora recovery, and globally unique active/unresolved covenant-reserve units with exact replay and release-only reuse | `tests/fixtures/nipmkt/ark-rail-v1.json`, `tests/fixtures/provider/ark-runtime-v1.json`, `crates/immortal-core/src/ark.rs`, `crates/immortal-core/tests/ark.rs`, `crates/immortal-client/src/ark.rs`, `crates/immortal-client/tests/ark.rs`, migration 0004, and `crates/immortal-provider/tests/provider_ark_store_postgres.rs` |
...
| Adversarial-lab #18 closed-world execution: exact isolated two-provider/two-relay topology, 46 routing/failure/doomsday/cooperative cases including ten Liquid cases and three opt-in zero-confirmation downgrade attacks, independent evidence views, cleanup laws, and explicit deployment/replacement exclusions; all 46 plus bounded aggregate generation passed from pushed `main` `3d65af1` | `tests/fixtures/lab/adversarial-v1.json`, `scripts/test-lab-adversarial.sh --all`, and [`records/2026-08-06-adversarial-regtest-3d65af1.json`](records/2026-08-06-adversarial-regtest-3d65af1.json) |
