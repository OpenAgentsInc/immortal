...
| Manual two-process M8 soak: cross-process notification burst, sustained replacement writes, WebSocket connection churn, relay RSS, and Postgres size/live/dead-tuple/connection bounds | `tests/fixtures/nip01/soak-plan.json`, `crates/immortal-relay/tests/soak_postgres.rs`, and `scripts/test-soak.sh` |
