...
| Adversarial-lab provisioning: opt-in-RBF creation/replacement, two provider-owned CLN nodes with mandatory probed hold RPCs, a separate wallet CLN node, balanced three-edge topology, loopback two-relay/two-provider allocations, ownership-checked teardown, an implemented feature-gated LND provider process gate, and hook-only elementsd/arkd registry entries without credential transfer | `tests/fixtures/lab/provisioning-v1.json`, `scripts/lab-bitcoind.sh`, `scripts/lab-cln.sh`, `scripts/lab-extensions.sh`, `scripts/lab-topology.sh`, and `scripts/test-lab-provisioning.sh` (static/fixture gate; clean-machine funded execution remains required by #32) |
...
| Adversarial-lab #18 closed-world execution: exact isolated two-provider/two-relay topology, 33 routing/failure/doomsday/cooperative cases, independent evidence views, cleanup laws, and explicit deployment/replacement exclusions; all 33 passed from pushed `main` | `tests/fixtures/lab/adversarial-v1.json`, `scripts/test-lab-adversarial.sh --all`, and [`records/2026-08-05-adversarial-regtest-67efec7.json`](records/2026-08-05-adversarial-regtest-67efec7.json) |
| Swap-network migration boundary: provider drain without abandoning active sessions, public active-session metrics, relay-independent service lifetime, immutable client provider-origin pin, seven-route GET-only live shadow, atomic new-session cutover, and rollback without in-flight failover | `tests/fixtures/nipmkt/swap-network-migration-v1.json`, `scripts/boltz-readonly-shadow.py`, `scripts/test-provider-deployment-assets.sh`, and `scripts/test-swap-network-migration.sh` |
