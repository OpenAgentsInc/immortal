...
| OpenAgents NIP-MKT public Provider Profile, Offering, Profile Descriptor, and Public Market Receipt required tags, enums, identifiers, content bounds, and ordinary NIP-01 addressable replacement classification | `tests/fixtures/nipmkt/public-heads.json` and `tests/mkt_fixtures.rs` |
| OpenAgents NIP-MKT private RFQ through Close immutable-coordinate admission, exact replay, changed-ID/signature conflicts, deletion/expiry persistence, concurrent first writers, and complete `39600-39699` addressable classification | `tests/fixtures/nipmkt/immutability.json`, `tests/mkt_immutability_model.rs`, and the live store contract |
| OpenAgents NIP-MKT common public/private bounds, duplicate-free JSON, private envelope agreement, stable error codes, exact raw signed-record bytes and crypto, explicit profile support, gateway wire-byte bounds, and observable/client boundary | `tests/fixtures/nipmkt/common-grammar.json`, `tests/mkt_common_fixtures.rs`, gateway wire tests, the live store contract, and `docs/protocol/nip-mkt-validation.md` |
