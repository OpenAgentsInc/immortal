...
| MKT-SWP Ark verification core: exact Arkade/Bark operator identity and public-policy binding, operator/network pair identity, bounded signed transaction-tree or transaction-chain traversal, observed-root authority, Taproot signature and VTXO path verification, expiry safety, and byte-stable fixture replay | `tests/fixtures/nipmkt/ark-rail-v1.json`, `crates/immortal-core/src/ark.rs`, and `crates/immortal-core/tests/ark.rs` |
