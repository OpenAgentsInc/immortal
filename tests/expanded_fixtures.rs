...
fn nip17_gift_wrap_and_inbox_list_fixture_corpus() {
    let fixture: RoutingFixture =
        serde_json::from_str(include_str!("fixtures/nip17/routing.json")).unwrap();
...
fn nip29_group_action_fixture_corpus() {
    let fixture: GroupFixture =
        serde_json::from_str(include_str!("fixtures/nip29/groups.json")).unwrap();
    for case in fixture.cases {
        let event = example_event(case.kind, case.tags, "");
...
fn nip50_search_filter_fixture_corpus() {
    let fixture: SearchFixture =
        serde_json::from_str(include_str!("fixtures/nip50/search.json")).unwrap();
...
fn nip70_protected_tag_fixture_corpus() {
    let fixture: ProtectedFixture =
        serde_json::from_str(include_str!("fixtures/nip70/protected.json")).unwrap();
    for case in fixture.cases {
...
fn nip98_http_auth_fixture_contract() {
    let fixture: Value =
        serde_json::from_str(include_str!("fixtures/nip98/http-auth.json")).unwrap();
