...
fn nip01_known_event_id_and_signature() {
    let fixture: EventFixture = serde_json::from_str(include_str!("fixtures/nip01/events.json"))
...
fn nip01_crypto_checks_fail_closed() {
    let mut fixture: EventFixture =
        serde_json::from_str(include_str!("fixtures/nip01/events.json")).unwrap();
...
fn nip01_filter_fixture_corpus() {
    let fixture: FilterFixture = serde_json::from_str(include_str!("fixtures/nip01/filters.json"))
...
fn nip01_replacement_race_fixture_corpus() {
    let fixture: ReplacementFixture =
        serde_json::from_str(include_str!("fixtures/nip01/replacement.json")).unwrap();
    for case in fixture.cases {
...
fn nip09_deletion_and_tombstone_fixture_corpus() {
    let fixture: DeletionFixture =
        serde_json::from_str(include_str!("fixtures/nip09/deletion.json")).unwrap();
...
fn nip40_expiration_fixture_corpus() {
    let fixture: ExpirationFixture =
        serde_json::from_str(include_str!("fixtures/nip40/expiration.json")).unwrap();
    for case in fixture.cases {
...
fn nip40_expiration_is_inclusive_and_admission_rejects_expired() {
    let mut event: Event =
        serde_json::from_str::<EventFixture>(include_str!("fixtures/nip01/events.json"))
            .unwrap()
            .event;
